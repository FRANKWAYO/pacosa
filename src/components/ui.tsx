...
export function Wordmark({ light = false, compact = false }: { light?: boolean; compact?: boolean }) {
  return (
    <div className="flex items-center gap-3">
      <Crest size={compact ? 38 : 46} />
      <div className="leading-none">
        <div className={`font-display font-bold tracking-[0.08em] ${compact ? "text-2xl" : "text-3xl"} ${light ? "text-paper" : "text-pine-900"}`}>PACOSA</div>
        <div className={`text-[0.6rem] font-bold tracking-[0.34em] uppercase mt-1 ${light ? "text-gold-300" : "text-gold-600"}`}>Cadets Association</div>
      </div>
    </div>
  );
}
