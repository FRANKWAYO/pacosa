...
export function JoinPage() {
  ...
  const submit = () => {
    ...
    notifyAdmins(
      "application",
      `New membership application — ${member.fullName}`,
      `${member.fullName} (${member.region}) applied for PACOSA membership.\nReference: ${ref}\nReview it in the admin panel under Applications.`
    );
...
export function ContactPage() {
  ...
  const submit = () => {
    ...
    window.setTimeout(() => {
      ...
      notifyAdmins(
        "message",
        `New contact message — ${f.subject.trim()}`,
        `From: ${f.name.trim()} <${f.email.trim()}>${f.phone.trim() ? ` (${f.phone.trim()})` : ""}\n\n${f.body.trim()}`
      );
